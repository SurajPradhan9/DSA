...
public class Main {
    public static void main(String[] args) {
        ...
        ParentClass obj5=new ChildClass(); // we are over-ridding the greeting method but instead of printing the greeting method that is present in childClass it is printing parentClass greeting method only(All the static methods that being inherited imagine they were being inherited there is no point in over-ridding them in child class because the method in the parent class will always run no matter from which object we call it because it is not dependent on object it is static i.e the reason that static methods that is in childClass is not being called because the main one that is in the parentClass is called )eventhough it can be inherited we can use it via childClass object i.e ChildClass obj=new ChildClass()it will print the same thing (In simple words - static methods can be inherited but cannot be over-ridden because it is not dependent on objects)(over-ridding depends on objects ,static does-not depends on objects,hence static methods cannot be over-ridden)so dynamic method dispatch cannot be happen if they are static
        obj5.greeting();
        ...
        System.out.println(parentClass5.l); // we can only access parent class members we cant access childClass members i.e parentClass5.weight
